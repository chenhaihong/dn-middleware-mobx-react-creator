...
class Creator {
  ...
  getControllerFiles(name) {
    ...
    return {
      ...
      ...(context.user_inputs.is_page ? this.getPageFiles(name) : {}),       // 加入生成page模板参数
      ...(context.user_inputs.is_service ? this.getServiceFiles(name) : {}), // 加入生成service模板参数
      ...(context.user_inputs.is_mock ? this.getMockFiles(name) : {})        // 加入生成mock模板参数
    };
  }
